...
<style scoped>
.select-chip-wrap { position: relative; display: inline-flex; }
.chip-badge {
...
.chip-dec:hover { background: rgba(255,255,255,0.45); }
.slot-full-hint { font-size: 11px; color: var(--ink-3); margin: 6px 0 0; }
.upgrade-row { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
.upgrade-label { font-size: 13px; font-weight: 500; color: var(--ink); }
.upgrade-cost { margin-left: 6px; font-family: var(--font-en); font-weight: 700; color: var(--accent); }
.upgrade-slot-hint { font-size: 11px; color: var(--ink-3); font-family: var(--font-en); }
.opt-chip:disabled { opacity: 0.4; cursor: not-allowed; transform: none; }
...
.mod-group-toggle:hover .mod-group-label { color: var(--ink); }
.addon-hint { flex: 1; }
.toggle-arrow {
